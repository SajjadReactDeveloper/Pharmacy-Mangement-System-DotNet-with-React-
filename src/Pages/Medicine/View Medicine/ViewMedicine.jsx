...
export default function ViewMedicine() {
  ...
  React.useEffect(() => {
    const getData = async () => {
      const res = await axios.get(`https://localhost:7107/SuperVillain/medicine`);
      setCars(res.data);
    };
    getData();
  }, []);
  ...
  const deleteComplaint = async (id) => {
    try {
      const res = await axios.delete(`https://localhost:7107/SuperVillain/deleteMedicine/${id}`);
      window.location.href = "/medicine";
    } catch (error) {}
  };

  const handleClickOpen = (data) => {
    navigate("/medicine/edit", {state: data});
  };
